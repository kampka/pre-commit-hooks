...
let
  hookScript = {
    ...
    ${scalafmtPkg}/bin/scalafmt --list --non-interactive ${lib.optionalString (config != null) ''--config ${config}''} ${lib.optionalString (configStr != null) ''--config-str "${configStr}"''} $changedFiles
    '';
in {
